...
const contentCpm = {
  TikTok: { post: 14, story: 8, video: 20, UGC: 22 },
  Instagram: { post: 16, story: 9, video: 22, UGC: 20 },
  YouTube: { post: 18, story: 8, video: 28, UGC: 24 },
}

const followerFloorPer1k = {
  TikTok: { post: 4, story: 2, video: 5, UGC: 5 },
  Instagram: { post: 5, story: 3, video: 6, UGC: 5 },
  YouTube: { post: 5, story: 2, video: 8, UGC: 6 },
}

const estimatedViewRatio = {
  TikTok: { post: 0.18, story: 0.08, video: 0.32, UGC: 0.24 },
  Instagram: { post: 0.14, story: 0.08, video: 0.22, UGC: 0.18 },
  YouTube: { post: 0.1, story: 0.04, video: 0.1, UGC: 0.08 },
}
...
function getEngagementMultiplier(engagementRate) {
  ...
  if (engagement >= 8) return 1.18
  if (engagement >= 5) return 1.1
  if (engagement >= 3) return 1
  if (engagement >= 1.5) return 0.93
  return 0.86
}
...
export function calculatePrice(input = {}) {
  ...
  const usageFee = deliverablesAdjusted * (usageRightsFeePercent[usageRights] ?? 0)
  const exclusivityFee = deliverablesAdjusted * (exclusivityFeePercent[String(exclusivityDays)] ?? 0)
  const rushFee = deliverablesAdjusted * (rushFeePercent[turnaround] ?? 0)
  const revisionFee = Math.max(0, revisionsNum - 1) * 25
  const proTotal = deliverablesAdjusted + usageFee + exclusivityFee + rushFee + revisionFee
